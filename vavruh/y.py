...
def integ_y(xi,y,xi1,coef):
    ...
    for i in range(len(xi)):
        try:
            yy.append(1/xi[i]**2*sp.integrate.quad(alpha_y,0,xi[i],args=(y[i],xi1,coef))[0])
        except ZeroDivisionError:
            yy.append(0)
    return yy
def pade_mu(x):
    ...
    dbc.Row([
        ...
        dbc.Col([
            ...
            dbc.Col([dbc.Label("a0"),
                     dbc.Input(type="number",min=-10,max=10,step=0.00001,id="a0",value=0.0001),]),
            dbc.Col([dbc.Label("b0"),
                     dbc.Input(type="number",min=-10,max=10,step=0.00001,id="b0",value=0.000448987),])
                ]),
            dbc.Row([
            dbc.Col([dbc.Label("a1"),
                     dbc.Input(type="number",min=-10,max=10,step=0.0001,id="a1",value=-0.0011),]),
            dbc.Col([dbc.Label("b1"),
                     dbc.Input(type="number",min=-10,max=10,step=0.0001,id="b1",value=-0.00399509),])
                ]),

            dbc.Row([
            dbc.Col([dbc.Label("a2"),
                     dbc.Input(type="number",min=-10,max=10,step=0.001,id="a2",value=0.035),]),
            dbc.Col([dbc.Label("b2"),
                     dbc.Input(type="number",min=-10,max=10,step=0.001,id="b2",value=0.0745353)])
                ]),
             dbc.Row([
            dbc.Col([dbc.Label("a3"),
                     dbc.Input(type="number",min=-10,max=10,step=0.001,id="a3",value=-0.53596),]),
            ...
            dbc.Col([dbc.Label("a4"),
                     dbc.Input(type="number",min=-10,max=10,step=0.001,id="a4",value=4.97482),]),
            dbc.Col([dbc.Label("b4"),
                     dbc.Input(type="number",min=-10,max=10,step=0.001,id="b4",value=7.08559),])
                ]),
            html.Br(),
            dbc.Row([
...
def update_figure1(fig,n,a0,a1,a2,a3,a4,b0,b1,b2,b3,b4):
    if n==0:
        ...
        dF=sm.lambdify(x_,dF)
        xi_me,y_me=calc(6.897,g1,mu_ser,a0,a1,a2,a3,a4,b0,b1,b2,b3,b4)
        
        xi_me2,y_me2=error(6.897,g2,1e-2,mu_ser,a0,a1,a2,a3,a4,b0,b1,b2,b3,b4)
        
        i_me=integ_y(xi_me,y_me,6.897,coef)
        i_me2=integ_y(xi_me2,y_me2,xi_me2[-1],coef)
        ...
        def g3(xi,y,u,xi1,mu_ser,coef):
            try:
                return -2/xi*u-y**3*F(xi/xi1,mu_ser,*coef)**2-(F(0,mu_ser,*coef)/xi1)*dF(xi/xi1)*i_apr(xi)
            except ZeroDivisionError:
                return -y**3
        xi_me3,y_me3=error(xi_me2[-1],g3,1e-2,mu_ser,a0,a1,a2,a3,a4,b0,b1,b2,b3,b4)
        #print(xi_me3[-1])
        ...
        fig1.update_layout(
            ...
            showlegend=True,
             xaxis=dict(
                tickmode = 'array',
                tickvals = np.linspace(0, xi_me3[-1], 5),
                #ticktext = ['0', "0.25", '0.5', '0.75', '1']
             ),
            width=750,
            height=600,
...
def update_figure2(fig,a0,a1,a2,a3,a4,b0,b1,b2,b3,b4):
  ...
  #  print(i)
   ...
   # input()
    def M_h0(x):
        return x**2*(apy_t0(x)**3)*(pade_mu(x/xi1)/pade_mu(0))*apr_x(x/xi1)
    def M_h1(x):
